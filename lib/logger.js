...
class Logger {
  ...
  _formatMessage(level, message, metadata) {
    ...
    const logLevel = this._config.includeLevel ? this.levels[level]?.name || level.toUpperCase() : null;
    ...
    if (this._config.format === 'json') {
      ...
      if (logLevel) parts.push(`[${logLevel}]`);
      parts.push(message);
  ...
  child(defaultMetadata = {}) {
    ...
    return {
      ...
      log: (level, message, metadata = {}) => parent.log(level, message, { ...defaultMetadata, ...metadata }),
    };
  }
